...
import {
  buildCmdCtx, buildQryCtx, makePMRepo,
  idParam, errorBody,
...
const meterRuleSchema = z.object({
 ...
 */
function toCalendarRuleInput(
  r: NonNullable<z.infer<typeof calendarRuleSchema>>,
): { frequency: 'daily' | 'weekly' | 'monthly' | 'quarterly' | 'annually'; interval: number; dayOfWeek?: number; dayOfMonth?: number; month?: number } {
  return {
...
const pmCrudRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post(
    ...
    {
      schema: {
        description: 'Create a new PM schedule with tasks. Initial nextDueAt is computed automatically for CALENDAR type.',
        tags:     ['pm-schedules'],
        security: [{ bearerAuth: [] }],
        body: {
          type: 'object',
    ...
    async (request, reply) => {
      ...
      const id = await handler.handle({
        ...
        ...(body.calendarRule       !== undefined && { calendarRule:       toCalendarRuleInput(body.calendarRule) }),
        ...
        ...(body.defaultAssigneeIds !== undefined && { defaultAssigneeIds: body.defaultAssigneeIds }),
    ...
    '/ai/suggest',
    {
      schema: {
        ...
        body: {
          ...
          required: ['assetType'],
          properties: {
            assetType:    { type: 'string', minLength: 1, maxLength: 200, description: 'e.g. "Centrifugal Pump"' },
  ...
  fastify.get<{ Params: { id: string } }>(
    ...
    {
      schema: {
        description: 'Full PM schedule detail: task list, calendarRule/meterRule, and trigger history from the audit log.',
        tags:     ['pm-schedules'],
        security: [{ bearerAuth: [] }],
        params:   idParam,
        response: {
  ...
  fastify.patch<{ Params: { id: string } }>(
    ...
    {
      schema: {
        description: 'Update mutable PM schedule fields. When calendarRule changes, nextDueAt is recomputed.',
        tags:     ['pm-schedules'],
        security: [{ bearerAuth: [] }],
        params:   idParam,
        body: {
    ...
    async (request, reply) => {
      ...
      await handler.handle({
        ...
        ...(body.calendarRule       !== undefined && {
          calendarRule: body.calendarRule !== null
            ? toCalendarRuleInput(body.calendarRule)
            : null,
        }),
        ...(body.meterRule          !== undefined && { meterRule:          body.meterRule }),
  ...
  fastify.delete<{ Params: { id: string } }>(
    ...
    {
      schema: {
        description: 'Deactivate a PM schedule (soft delete). Existing WOs created from this schedule are unaffected.',
        tags:     ['pm-schedules'],
        security: [{ bearerAuth: [] }],
        params:   idParam,
        response: {
