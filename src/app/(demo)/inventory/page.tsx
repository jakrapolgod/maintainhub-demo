  ...
  return (
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-3 gap-3">
        {(
          [
            ...
            ['Low Stock', String(lowCount), true],
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b text-left text-xs text-muted-foreground">
            {['Part #', 'Name', 'Category', 'Qty', 'Min', 'Unit Cost', 'Location', ''].map((h) => (
        ...
        <tbody className="divide-y divide-border">
          {rows.map((p) => (
            <tr
              ...
              className={cn(
                'transition-colors',
                p.quantity <= p.minStock ? 'bg-amber-50' : 'hover:bg-muted/40',
              )}
            >
              ...
              <td className="py-2 pr-3">
                <span
                  className={cn('rounded px-1.5 py-0.5 text-xs font-medium', CAT_COLOR[p.category])}
                >
                  {p.category}
                </span>
              </td>
              ...
              <td className="py-2 pr-3 text-xs text-muted-foreground">{p.warehouse}</td>
              <td className="py-2">
                <Button
                  variant="outline"
                  size="sm"
      ...
      <Dialog open={!!adj} onOpenChange={(v) => !v && setAdj(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Adjust Stock — {adj?.name}</DialogTitle>
          </DialogHeader>
          <div className="space-y-3 py-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-xs text-muted-foreground">
                current: {adj?.quantity} → new: {Math.max(0, (adj?.quantity ?? 0) + delta)}
              </span>
            </div>
            <Input
              placeholder="Reason (e.g. used in WO-003)"
              value={reason}
          ...
          <DialogFooter>
            ...
            <Button onClick={applyAdj} disabled={delta === 0 || !reason}>
              Apply
            </Button>
          </DialogFooter>
        </DialogContent>
